...
def calculate_texture_features_n_hog(image):
    ...
    lbp = feature.local_binary_pattern(gray, P=8, R=3, method='uniform')
...
def extract_features_advanced(image_np_array):
    ...
    image = pre_process_image(image_np_array)
    
    # hu_moments = cv2.HuMoments(cv2.moments(cv2.cvtColor(image, cv2.COLOR_BGR2GRAY))).flatten()
    # texture_features_n_hog = calculate_texture_features_n_hog(image)
    # color_hist_hsv = calculate_histogram(cv2.cvtColor(image, cv2.COLOR_BGR2HSV), channels=[0, 1, 2])
    ...
    return np.concatenate((color_moments, color_moments_hsv, color_moments_lab, texture_features_n_hog))
...
if __name__ == "__main__":
    IMG_SIZE = (227, 227)
    TEST_DATA_DIR = f"E:/Backup Mario/Coding Room/Rice_Disease/datasets/result/250902 2018 Mixed On Field No Duplicate Normal Indo/test_images"
    MODEL_PATH = f"E:/Backup Mario/Coding Room/Rice_Disease/Results/250902 2018 Mixed On Field No Duplicate Normal Indo/svm_model.pkl"
    SCALER_PATH = f"E:/Backup Mario/Coding Room/Rice_Disease/Results/250902 2018 Mixed On Field No Duplicate Normal Indo/scaler.pkl"
